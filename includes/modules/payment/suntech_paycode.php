...
$suntech_payment = 'suntech_paycode';
...
class suntech_paycode
{
    ...
    {
        $this->suntech_paycode();
        $this->suntech_payment = 'suntech_paycode';
    }

    function suntech_paycode()
    {
    }
        ...
        if ($store_type != '') {
            ...
            if (($error_code === "00") || ($error_code === "0")) {
                $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') .
                    ...
                    " WHERE order_id = '" . $order_info['order_id'] . "'";
                $GLOBALS['db']->query($sql);
                $note = $GLOBALS['_LANG']['suntech_paid'] . $GLOBALS['_LANG']['suntech_pay_type_' . $_POST['PayType']];
            ...
            $check_value = strtoupper(sha1($_POST["web"] . $store_pwd . $_POST['buysafeno'] . intval($_POST["MN"]) . $_POST['paycode']));
            if ($check_value != $_POST['ChkValue']) {
                return false;
            };
            ...
            if ($order_info['order_status'] != OS_CONFIRMED && isset($_SESSION['payment_duedate'])) {
                ...
                $note .= '\n' . sprintf($GLOBALS['_LANG']['suntech_paycode_msg'], $_POST['paycode']);
                setOrderPayName($order_info['order_id'], sprintf($GLOBALS['_LANG']['suntech_paycode_code'], $_POST['paycode']));
                setOrderFeedBack($order_info['user_id'], $note, $order_info['order_id']);
                $order_amount = $order_info['order_amount'];
                order_paid($log_id, 1, $note);
