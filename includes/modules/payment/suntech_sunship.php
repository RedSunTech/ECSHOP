...
$suntech_payment = 'suntech_sunship';
...
class suntech_sunship
{
    ...
    {
        $this->suntech_sunship();
        $this->suntech_payment = 'suntech_sunship';
    }

    function suntech_sunship()
    {
    }
    ...
    function respond()
    {
        ...
        if ($store_type != '') {
            ...
            $check_value = strtoupper(sha1($_POST["web"] . $store_pwd . $_POST['buysafeno'] . intval($_POST["MN"]) . $error_code));
            if ($check_value != $_POST['ChkValue']) {
                return false;
            };
            ...
            if (($error_code === "00") || ($error_code === "0")) {
                $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') .
                    " SET shipping_status = '" . SS_RECEIVED . "', shipping_time = '" . gmtime() . "'" .
                    " WHERE order_id = '" . $order_info['order_id'] . "'";
                $GLOBALS['db']->query($sql);
                $note = $GLOBALS['_LANG']['suntech_paid'];
                setOrderFeedBack($order_info['user_id'], $note, $order_info['order_id']);
            ...
            $check_value = strtoupper(sha1($_POST["web"] . $store_pwd . $_POST['buysafeno'] . intval($_POST["MN"]) . $cargo_no));
            if ($check_value != $_POST['ChkValue']) {
                return false;
            };
