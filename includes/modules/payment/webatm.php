...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/webatm.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['config']  = array(
        array('name' => 'webatm_account',           'type' => 'text',   'value' => ''),
        array('name' => 'webatm_password', 'type' => 'text', 'value' => '')
    );
...
class webatm
{
    ...
    function get_code($order, $payment)
    {
        ...
        $form.='<input type="hidden" name="web" value="'.$payment["webatm_account"].'">';//商店代號
        ...
        $form.='<input type="hidden" name="note1" value="webatm">';//備註1 maxlength="4000"
        $form.='<input type="hidden" name="note2" value="'.$order['log_id'].','.strtoupper(sha1($payment["webatm_account"].$payment["webatm_password"].$order['order_sn'].$order['log_id'].$order_amount)).'">';//備註2 maxlength="200"
        $form.='<input type="hidden" name="Card_Type" value="">';//交易類別
        $form.='<input type="hidden" name="ChkValue" value="'.strtoupper(sha1($payment["webatm_account"].$payment["webatm_password"].$order_amount)).'">';//備註2 maxlength="200"
    ...
    function respond()
    {
        ...
        $payment  = get_payment('webatm');
        $order_sn = $_GET['Td'];
        $order = explode(',',urldecode($_GET['note2']));
        $log_id = $order[0];
        if ($order[1] != strtoupper(sha1($payment["webatm_account"].$payment["webatm_password"].$order_sn.$log_id.intval($_GET["MN"])))) {
        	return false;
        }
        if ($_GET['ChkValue'] != strtoupper(sha1($payment["webatm_account"].$payment["webatm_password"].$_GET['buysafeno'].intval($_GET["MN"]).$_GET['errcode']))) {
            return false;
        }
        /* 检查支付的金额是否相符 */
